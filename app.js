        ...
        dropdownItem.forEach((d) => {
          d.addEventListener("click", (e) => {
            ...
            for (let i = 0; i < dropdownMenu.childElementCount; i++) {
              if (
                dropdownMenu.children[i].firstElementChild.classList.contains(
                  "active"
                )
              ) {
                dropdownMenu.children[i].firstElementChild.classList.remove(
                  "active"
                );
                break;
              }
    ...
    cartHandler: function () {
      ...
      addFood.forEach((a) => {
        a.addEventListener("click", () => {
          ...
          const countFoodElem = a.nextElementSibling.nextElementSibling;
          if (matched != -1) {
            this.data.orders[matched].quantity += 1;
            countFoodElem.textContent = this.data.orders[matched].quantity;
          } else {
      ...
      removeFood.forEach((r) => {
        r.addEventListener("click", () => {
          const food_value =
            r.parentElement.previousElementSibling.getAttribute("data-value");
